...
const RESERVED_SHIPPING_IDS = new Set(["store", "pickup", "lalamove", "gosend"]);

function normalizeCustomMethods(list) {
  ...
  for (const raw of list) {
    ...
    if (!id || RESERVED_SHIPPING_IDS.has(id) || !/^custom_[a-zA-Z0-9_-]{1,64}$/.test(id)) {
    ...
    out.push({
      ...
      flatFee: roundFee(raw.flatFee),
      perKmRate: Math.max(0, Number(raw.perKmRate) || 0),
      freeAboveSubtotal: Math.max(0, Number(raw.freeAboveSubtotal) || 0),
      requireGpsQuote: raw.requireGpsQuote !== false,
...
function mergeShippingSettings(stored) {
  ...
  return {
    ...
    customMethods: normalizeCustomMethods(stored.customMethods),
  };
}
...
function quoteFlatOrPerKm({ cfg, provider, productsSubtotal, origin, destination }) {
  const minFee = roundFee(cfg.flatFee ?? 0);
...
function quoteStore({ settings, productsSubtotal, origin, destination }) {
  return quoteFlatOrPerKm({
    cfg: { ...(settings.storeDelivery || {}), label: settings.storeDelivery?.label || "Kirim mobil toko" },
    provider: "store",
    productsSubtotal,
    origin,
    destination,
...
function getShippingOptions(settings) {
  ...
  const options = [];
  if (s.pickup?.enabled !== false) {
    options.push({
      id: "pickup",
      label: s.pickup?.label || "Ambil sendiri",
      description: s.pickup?.note || "Ambil di gudang / toko, tanpa ongkir",
      requiresQuote: false,
      requireGpsQuote: false,
      autoFee: 0,
      quoteNeedsDestination: false,
    });
  }
  if (s.storeDelivery?.enabled !== false) {
    const perKmRate = Number(s.storeDelivery?.perKmRate) || 0;
    const requireGpsQuote = s.storeDelivery?.requireGpsQuote !== false;
    options.push({
      id: "store",
      label: s.storeDelivery?.label || "Kirim mobil toko",
      description:
        requireGpsQuote && perKmRate > 0
          ? `Dihitung dari jarak gudang × Rp ${perKmRate.toLocaleString("id-ID")}/km`
          : s.storeDelivery?.note || "Diantar kurir toko",
      requiresQuote: requireGpsQuote,
      requireGpsQuote,
      autoFee: roundFee(s.storeDelivery?.flatFee ?? 0),
      distanceBased: requireGpsQuote && perKmRate > 0,
      quoteNeedsDestination: requireGpsQuote && perKmRate > 0,
    });
  }
  ...
  for (const custom of s.customMethods || []) {
    if (custom.enabled === false) continue;
    const perKmRate = Number(custom.perKmRate) || 0;
    const requireGpsQuote = custom.requireGpsQuote !== false;
    options.push({
      id: custom.id,
      label: custom.label,
      description:
        requireGpsQuote && perKmRate > 0
          ? `Dihitung dari jarak gudang × Rp ${perKmRate.toLocaleString("id-ID")}/km`
          : custom.note || `Tarif flat ${roundFee(custom.flatFee).toLocaleString("id-ID")}`,
      requiresQuote: requireGpsQuote,
      requireGpsQuote,
      autoFee: roundFee(custom.flatFee),
      distanceBased: requireGpsQuote && perKmRate > 0,
      quoteNeedsDestination: requireGpsQuote && perKmRate > 0,
    });
  }
...
async function calculateShippingQuote({
  ...
  if (method === "pickup") {
    if (merged.pickup?.enabled === false) {
    ...
    return {
      ...
      source: "pickup",
      estimated: false,
      label: merged.pickup?.label || "Ambil sendiri",
      note: merged.pickup?.note || "Ambil di gudang / toko, tanpa ongkir",
      provider: "pickup",
    };
  }
  ...
  if (method === "store") {
    const requireGpsQuote = merged.storeDelivery?.requireGpsQuote !== false;
    const perKmRate = Number(merged.storeDelivery?.perKmRate) || 0;
    if (requireGpsQuote && perKmRate > 0) {
  ...
  const custom = (merged.customMethods || []).find((m) => m.id === method && m.enabled !== false);
  if (custom) {
    const requireGpsQuote = custom.requireGpsQuote !== false;
    const perKmRate = Number(custom.perKmRate) || 0;
    if (requireGpsQuote && perKmRate > 0) {
