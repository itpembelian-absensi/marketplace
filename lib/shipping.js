...
async function quoteLalamove({ settings, origin, destination, address }) {
  ...
  const sandbox = process.env.LALAMOVE_SANDBOX === "true" || settings.lalamove?.sandbox !== false;
  ...
  const price = data.priceBreakdown?.total || data.price?.total || data.totalFee || data.fee;
...
async function quoteGoSend({ settings, origin, destination }) {
  const baseUrl = (process.env.GOSEND_API_BASE || "").replace(/\/$/, "");
  const clientId = process.env.GOSEND_CLIENT_ID || "";
  const passKey = process.env.GOSEND_PASS_KEY || "";
  const estimatePath =
  ...
  if (!res.ok) {
    throw new Error(json?.message || `GoSend API error (${res.status})`);
  }

  const instant = json.Instant || json.instant || json.data?.Instant;
  const sameDay = json.SameDay || json.sameDay;
  const block = instant || sameDay || json;
  const priceBlock = block?.price || block;
  const fee = roundFee(
    priceBlock?.total_price || priceBlock?.totalPrice || priceBlock?.price || json.total_price
  );
  if (!fee) {
  ...
  return {
    fee,
    distanceKm: Number(json.distance) || null,
    source: "gosend_api",
    estimated: false,
    label: `GoSend ${methodKey}`,
    note: "",
    provider: "gosend",
  };
}
